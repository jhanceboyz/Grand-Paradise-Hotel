<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage1.master" AutoEventWireup="true" CodeFile="ResturantDesc.aspx.cs" Inherits="RoomDesc" %>
...
<div class="row margin-top-30">
    ...
    <div class="col-lg-4 col-md-4 right-content padding-right-none xs-padding-left-none sm-padding-left-none"><div class="right_site_job">
        <div class="job sm-padding-bottom-40 xs-padding-bottom-40 xs-padding-top-30">
            ...
            <ItemTemplate>
                <h2><asp:Label ID="Label1" runat="server" Text='<%# Eval("Resturanttype") %>'></asp:Label></h2>
                <p><asp:Label ID="Label2" runat="server" Text='<%# Eval("description") %>'></asp:Label></p><br />
                <div class="project_details"><ul><li><i class='fa fa-check-circle'> Price: ₹ <asp:Label ID="Label4" runat="server" Text='<%# Eval("Price") %>'></asp:Label>/- For a Table</i></li></ul></div>               
                <asp:Button ID="Button1" runat="server" PostBackUrl='<%# Eval("id", "~/ResturantBooking.aspx?i={0}") %>' Text="Book Now" />  
            </ItemTemplate>
            </asp:DataList>           
        </div>       
    </div></div>    
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:DatabaseConnectionString %>" SelectCommand="SELECT * FROM [Resturanttable] WHERE ([id] = @id)">
